...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        ...
        finish  : $('<a>'+boton+'</a>').attr("onclick","guardar_encuesta_op(2)").addClass("btn btn-danger")
    };
			...
			if (evt.keyCode==13) {			
				...
				if (valor_pregunta==respuesta_pregunta) {
					if (pregunta_actual_co==83) {
						...
						$this.goForward();
						pregunta_actual_co++;						
						$("#ap_"+pregunta_actual_co).val("");
						$("#ap_"+pregunta_actual_co).focus();
					}					
				} else {
					$("#valor").html("Su respuesta es incorrecta, la respuesta correcta es: "+respuesta_pregunta+". Intente nuevamente");
					$("#ap_"+pregunta_actual_co).val("");
				}
			}
			return false;
    ...
    SmartWizard.prototype.goForward = function(){
		//alert($("#ap_"pregunta_actual_co).val());
...
// Default Properties and Events
	var titulos_es = new Array();
	var titulos_en = new Array();
	
	titulos_es = ("Proximo","Anterior","Enviar");
	titulos_en = ("Next","Last","Send");
	if (idioma=="es") {
