...
function startTimer(tiempo) {
	...
	function startTimer_co(tiempo, pruebas_presentadas,prueba) {
		...
		tiempo_co = setInterval(function () {
			...
			//alert(timer);
			if (tiemposec==-1) {
				alert("Pase ahora, a la siguiente actividad...");
				guardar_encuesta_co();
			}		
		}, 1000);
	}
	...
	function guardar_encuesta_ifh (tipo) {
		// GUARDANDO PRUEBA DE IFH
		var id_au;
		var f=eval("document.forms[0]");		
		var valor=0;
		...
		for (i=1; i<f.length; i++) {
			if (f.elements[i].name.indexOf("op_")>-1) {
				if (f.elements[i].checked==true) {
					valor=f.elements[i].name.substr(3);
					if (opciones==1 || opciones==60) {
						resultado+="&resultado"+j+"=";
						j++;
					}
					opciones++;
					resultado+=valor+"-"+f.elements[i].value+",";
				}
			}
		}
		//alert(resultado);
		/*if (tipo==0) {
			document.getElementById("finalizar_prueba_ultima").style.display="inline";
			document.getElementById("encuesta").style.display="none";
		} else*/
		//alert("../index_guardar_prueba_ifl?id_bateria="+bateria+"&resultado="+resultado+"&id_au="+id_au+"&orden="+orden);
		location.href="../index_guardar_prueba_ifl?id_bateria="+bateria+resultado+"&id_au="+id_au+"&orden="+orden;
	}	
	
	function guardar_encuesta_rv (tipo) {
	...
	function guardar_encuesta_hn (tipo) {
	...
	function guardar_encuesta_iep (tipo) {
	...
	function guardar_encuesta_co(tipo) {
		var id_au;
		id_au=document.forms[0].id_au.value;
		bateria=document.forms[0].bateria.value;
		orden=document.forms[0].orden.value;
	...
	function guardar_participante() {
		...
		texto_otro_pais=$("#texto_otro_pais").val();
		...
		var parametros = {
			...
			"texto_otro_pais" : texto_otro_pais,
		...
		$.ajax({
			...
			success:  function (data) {
				...
				if (URLactual.toString().indexOf("prueba_hi")>-1)
					location.href="../prueba_aceptacion/"+id_au+"-"+id_bateria;
	...
	function acepto() {
		...
		if (URLactual.toString().indexOf("epa")>-1)
			location.href="../prueba_epa_ejemplo/"+id_au+"-1-"+id_bateria;
		else
			location.href="../prueba_siguiente/"+id_au+"-1-"+id_bateria;		
	}
	
	function acepto_seuc() {		
	...
	function guardar_participante_resto() {
		...
		for (i=0; i<f.length; i++) {
			if (f.elements[i].name=="sexo")
				if (f.elements[i].checked==true)
					sexo = f.elements[i].value;
		}
		...
		for (i=0; i<f.length;i++) {
			if (f.elements[i].id.indexOf("valores_chk")>-1) {
				//alert(f.elements[i].id);valores_chk
				if (chk==0) {
					//valores_chk_2_1
					nombre=f.elements[i].id;
					nombre=nombre.substr(12);
					//alert(nombre);
					id_1=nombre.substr(0,nombre.indexOf("_"));
					//alert(f.elements[i].name+"="+f.elements[i].checked);
					if (f.elements[i].checked==true) {
						otros_valores+="valores_"+id_1+"-"+f.elements[i].value+".";
						chk++;
					}
			...
			} else if (f.elements[i].id.indexOf("cod_int_")>-1 || f.elements[i].id.indexOf("cod_area_")>-1 || f.elements[i].id.indexOf("telefono_")>-1) {
				if (f.elements[i].id.indexOf("cod_int_")>-1) {
					telefono="";
					valor_tlf=f.elements[i].value;
					if (valor_tlf.length==0) valor_tlf="000";
					if (valor_tlf.length==1) valor_tlf="00"+valor_tlf;
					if (valor_tlf.length==2) valor_tlf="0"+valor_tlf;
				}
				if (f.elements[i].id.indexOf("cod_area_")>-1) {
					valor_tlf=f.elements[i].value;
					if (valor_tlf.length==0) valor_tlf="000";
					if (valor_tlf.length==1) valor_tlf="00"+valor_tlf;
					if (valor_tlf.length==2) valor_tlf="0"+valor_tlf;					
				}
				if (f.elements[i].id.indexOf("telefono_")>-1) {
					valor_tlf=f.elements[i].value;
					if (valor_tlf.length==0) valor_tlf="0000000";
					if (valor_tlf.length==1) valor_tlf="000000"+valor_tlf;
					if (valor_tlf.length==2) valor_tlf="00000"+valor_tlf;
					if (valor_tlf.length==3) valor_tlf="0000"+valor_tlf;
					if (valor_tlf.length==4) valor_tlf="000"+valor_tlf;
					if (valor_tlf.length==5) valor_tlf="00"+valor_tlf;
					if (valor_tlf.length==6) valor_tlf="0"+valor_tlf;
				}
				telefono+=valor_tlf;
				//alert(telefono);
				if (f.elements[i].id.indexOf("telefono_")>-1) {
					var id = f.elements[i].id.substr(9);
					//alert(id);
					otros_valores+="valores_"+id+"-"+telefono+",";
				}
			} else {
				if (chk>0)
					otros_valores+=",";
		...
		//alert("index_guardar_participante?nombres="+nombres+"&apellidos="+apellidos+"&cedula="+cedula+"&sexo="+sexo+"&email="+email+"&nacionalidad="+nacionalidad+"&edad="+edad+"&nivel_formacion="+nivel_formacion+"&orientacion_area="+orientacion_area+"&orientacion_cargo="+orientacion_cargo+"&id_au="+id_au+"&otros_valores="+otros_valores);
		//return;
		...
		$.ajax({
			...
			success:  function (data) {
				location.href="../consultar_candidatos/0";
			}
		});
	}	
	...
	function colocar_respuesta_ejemplo(tipo, antes, despues, objeto) {
		if (tipo=="rv") {
			if (antes=="beber") {
				$("#texto_ejemplo").css("display","none");
				
				$("#boton_prueba").css("display","inline");
				$("#op").html("");
				$("#op").html("<strong><span style='color:blue;'>" + antes + "</span></strong> es a agua como comer es a <strong><span style='color:blue;'>" + despues + "</span></strong>");
			} else {
				$("#texto_ejemplo").css("display","inline");
				$("#texto_ejemplo").html("En este ejemplo, la elección correcta es  beber – pan, pues al colocar estas dos palabras, la frase adquiere una relación con pleno sentido.");
			...
			if (antes=="110") {
				...
				$("#texto_ejemplo").css("display","inline");
				$("#texto_ejemplo").html("En este ejemplo, el resultado correcto es 110.");				
			...
			if (name=="mas") {
				if (valor2==valor)
					alert("No puede tomar esta opcion")
				else {
					valor1=valor;
					for (i=1;i<5;i++)
						$("#label_"+name+"_"+i).css("backgroundColor","#c8c8c8");
					$("#label_"+name+"_"+valor).css("backgroundColor","blue");
					$("#boton_prueba").css("display","inline");					
				}
			} else {
				if (valor1==valor)
	...
	function verificar_click() {
		mouse_click=1;
	}
	
	function fortaleza_texto(valor) {
		var arreglo = new Array();
		var arreglo_div = new Array();
		
		arreglo[1]="(Menos del 10%  de las veces)";
		arreglo[2]="(Entre el 10%  y el 29% de las veces)";
		arreglo[3]="(Entre el 30%  y el 49% de las veces)";
		
		arreglo[4]="(Entre el 50 %  y el 69% de las veces)";
		arreglo[5]="(Entre el 70%  y el 90% de las veces)";
		arreglo[6]="(Más del 90%  de las veces)";
		
		arreglo_div[1]="des_com";
		arreglo_div[2]="des_mod";
		arreglo_div[3]="des_lig";
		
		arreglo_div[4]="acu_lig";
		arreglo_div[5]="acu_mod";
		arreglo_div[6]="acu_com";
		
		$("#boton_prueba").css("display","inline");
		$("#boton_prueba").css("float","right");
		
		for (i=1; i<7; i++)
			$("#"+arreglo_div[i]).css("display","none");	
		
		$("#"+arreglo_div[valor]).html(arreglo[valor]);	
		$("#"+arreglo_div[valor]).css("display","inline");
	}
	...
	function colocar_nro(id) {
		...
		if (nro_prueba==0) {
			...
			if (pregunta_actual_co==1)
				texto=valor+": La primera letra del apellido Ettedgui nos orienta hacia las gavetas 11 y 12, las cuales contemplan los apellidos que comienzan con la letra “E”, pero sólo la gaveta 12 presenta el intervalo que contiene la combinación “ET” pues abarca desde “ES” hasta “EZ”; por lo tanto, corresponde ubicarlo en esa gaveta.";
	...
	function ver_imagen_ejemplo(cantidad,numero, imagen) {
		...
		if (numero==8) {
			...
			$("#boton_prueba").css("display","none");
			$("#texto_ejemplo").css("display","inline");
			$("#texto_ejemplo").html("La línea se va inclinando progresivamente hacia la derecha. Al final de la secuencia la línea debe estar en posición horizontal.");
		}
		return false;
	}	
