...
@include('layout.header_encuesta')
	...
	{!! Form::open(array('url' => 'prueba_op/encuesta_5_1_4-'.$id_au, 'method' => 'post', 'class' =>  "form-horizontal", 'files'=>true)) !!}
			...
			<?php
				...
				$sql = "select distinct(tp.id), tp.nombre,btp.orden
						...
						where btp.id_bateria=5 and
							btp.id_tipo_prueba=tp.id and a.id=".$id_au." and btp.orden=1 
						order by orden";
				//echo $sql;
				$data = DB::select($sql);
				$i=1;
				foreach ($data as $data) {
					...
					$sql = "select distinct(i.id_prueba), i.texto
						...
						where i.id_prueba=tp.id and btp.id_bateria=5 and
							...
							and a.id_empresas=i.id_empresa and tp.id=$id_tp and btp.orden=1 order by 1";						
					//echo $sql; return;
					...
					?>
						<div id="instrucciones" align="center" class="col-lg-10 text-left" style="; margin:50px;font-size: 11pt; border-style: solid; border-color: #1abb9c; padding:10px; color: #1abb9c">
							...
							<div align="center">
								<input name="boton_prueba_{{ $i }}" id="boton_prueba_{{ $i }}" <?php if ($i>3) echo "style='display:none'"; else echo "style='display:inline'"; ?>  onclick="ver_encuesta_op(<?php echo $tiempo; ?>, 3, 3,<?=$id_au?>)" type="button" class="btn btn-primary" value="Comenzar">
							</div>
						</div>
					<?php
					$i++;
				...
				<!-- Smart Wizard -->
					...
					<div id="wizard_1" align="center" class="form_wizard wizard_horizontal" style="margin: 0px; padding: 0px; text-align: center">
						<?php 				
							...
							$sql = "select * from preguntas_op where id_pruebas=1 and id_preguntas>92";
							//echo $sql;
							$data=DB::select($sql);
						...
						<ul class="wizard_steps" style="top: 35px">
							<?php												
							for ($i=93; $i<($cantidad_1); $i++) { ?>
							<li>
								<a href="#step-<?php echo $i; ?>">
									<span class="step_no"><?php echo ($i-92); ?></span>
								</a>
							</li>
							<?php } ?>
						</ul>							
