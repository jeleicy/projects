...
<?php
	...
	//$sql = "delete from respuestas_op where id_autorizacion=233";
	//DB::delete($sql);	
	...
	$pruebas=0;
	//PRUEBA CO
	$sql = "select rco.id from respuestas_op rco, preguntas_op phl
...
?>
    ...
    {!! Form::open(array('url' => 'guardar_encuesta', 'method' => 'post', 'class' =>  "form-horizontal", 'files'=>true)) !!}
				...
				foreach ($data as $data) {
					...
					?>
						<div id="instrucciones_{{ $i }}" align="center" class="col-lg-10 text-left" style=" display:<?php echo $display[$i]; ?>; margin:50px;font-size: 11pt; border-style: solid; border-color: #1abb9c; padding:10px; color: #1abb9c">
							...
							<div align="center">
								<input name="boton_prueba_{{ $i }}" id="boton_prueba_{{ $i }}" <?php if ($i>3) echo "style='display:none'"; else echo "style='display:inline'"; ?>  onclick="ver_encuesta_op(<?php echo $tiempo; ?>, <?php echo $i; ?>, <?php echo $pruebas; ?>,<?php echo $id_au; ?>)" type="button" class="btn btn-primary" value="Comenzar">
							</div>
						</div>
					<?php
					$i++;
		...
		<div align="center">
		<?php
			$sql = "select * from bateria_tipo_prueba where id_bateria=5";
			$data=DB::select($sql);
			$prueba=1;
			foreach ($data as $data) {
				...
				?>
					<div id="encuesta_<?php echo $prueba; ?>" style="width: 100%; background: transparent; margin:0px; display: none; margin: auto; padding: 0px;">
						...
						@include('prueba_op.preguntas_5_'.$prueba, ['tiempo' => $tiempo])
					</div>					
				<?php
				$prueba++;
				if ($prueba==2)
