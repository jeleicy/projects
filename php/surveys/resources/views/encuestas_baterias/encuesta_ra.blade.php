...
<div id="contenedor" style="border:0px solid green;">
<!-- HEADER -->
<div id="header"><div id="header_fondo">
<div id="header_tit" align="left">
<span class="header_tit1">R</span><span class="header_tit2">azonamiento </span> <span class="header_tit1">A</span><span class="header_tit2">bstracto</span></div>
...
<?php 
	...
	$sql = "select tp.id, tp.url, tp.vista_tiempo from tipos_pruebas tp, bateria_tipo_prueba btp
			where btp.id_tipo_prueba=tp.id and btp.id_bateria=".$id_bateria." and orden=4";
	//echo $sql; return;
	$data=DB::select($sql);
...
<div id="instrucciones" align="center" style="display:inline;">
	...
	@include('encuestas_baterias.instrucciones',["boton"=> "1", "id_au"=>$id_au, "nro_inst"=>6, "titulo"=>$titulo,"instrucciones_antes"=>$instrucciones_antes,"instrucciones_despues"=>$instrucciones_despues, "tiempo"=>$tiempo, "funcion"=>"guardar_encuesta_ra"])
</div>	
<!-- INSTRUCCIONES -->
...
<div id="prueba{{ $tabindex }}" style="display: {{ $display }};">
	<div id="prueba2_op_cont">
		...
		<button type="button" class="prueba2"><br /><div id='img_respuesta_<?php echo $id_pregunta; ?>' style='width=80px; height:75px; padding: 0px; border-radius: 15px; margin: 0px; border-color: #c8c8c8; border: 2px solid'><img width='60' height='75' src="../imagenes/interrogacion.png"></div></button>
	</div>	
	
	<div id="prueba2_op_cont">
		<div id="prueba_num1"></div>
		<strong>Seleccione la opcion que considere mas adecuada...</strong>
		<br />
...
<?php $funcion="guardar_encuesta_ra"; ?>
...
@include('encuestas_baterias.finalizar_prueba',["funcion"=>$funcion,"final"=>0])
