...
<div id="instrucciones" align="center" style="display:inline;">
	<?php
		$sql = "select i.texto, tp.nombre, i.posicion
				...
				where tp.id=$id_tp and i.id_prueba=tp.id 
	...
	@include('encuestas_baterias.instrucciones',["boton"=> "1", "id_au"=>$id_au, "nro_inst"=>0, "titulo"=>$titulo,"instrucciones_antes"=>$instrucciones_antes,"instrucciones_despues"=>$instrucciones_despues, "tiempo"=>$tiempo, "funcion"=>"guardar_encuesta_co"])
</div>	
<!-- INSTRUCCIONES -->
					...
					<div id="wizard_2" align="center" class="form_wizard wizard_horizontal" style="top: 0px; margin: 0px; padding: 0px; text-align: center">
						<?php 
							...
							foreach ($data as $data) {
								...
								?>
								<?php
								echo '<div align="center" style="display: '.$display.'" id="prueba'.$tabindex.'">';
								...
								for ($i=0; $i<count($pregunta); $i++) {
									...
									if ($tabindex==1) {
										...
										$autofocus="";											
									echo "<script>respuesta_actual_co[".$num."]='".$tabindex."'; tabindex=".$tabindex."</script>";
									if ($i==0) {
										//$num++;
									...
									echo "<td><input autocomplete='off' tabindex='".$tabindex."' ".$autofocus." maxlength='2' class='clase_textos' width='35px' height='35px' class='clase' maxlength='2' style='width:50px; text-align: center' type='text' name='coord_".$id_pregunta."_".$complemento."' id='coord_".$id_pregunta."_".$complemento."' value='$valor'></td>";
									$tabindex++;
								}
