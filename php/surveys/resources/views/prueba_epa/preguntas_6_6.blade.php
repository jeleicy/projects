...
<script>nro_prueba=6; id_au=<?=$id_au?>; primera=new Array(); var num_actual=0; tabindex=1; var cantidad=0;</script>
...
<?php $i=0; ?>
			...
			<?php
				...
				$sql = "select distinct(tp.id), tp.nombre
						...
						where btp.id_bateria=$id_bateria and btp.orden=7 and
				...
				foreach ($data as $data) {
						...
						where i.id_prueba=tp.id and btp.id_bateria=$id_bateria and
							...
							and a.id_empresas=i.id_empresa and tp.id=$id_btp and btp.orden=7";
							//echo $sql; return;
		//$sql = "select i.texto, tp.nombre from instrucciones i, tipos_pruebas tp where tp.orden=7 and i.id_prueba=tp.id";
...
<div id="instrucciones" align="center" class="col-lg-10 text-left" style=" display:inline; margin:50px;font-size: 11pt; border-style: solid; border-color: #1abb9c; padding:10px; color: #1abb9c">
	...
	{{ FuncionesControllers::prueba_inter(9) }}
								...
								foreach ($data as $data) {
									...
									<div id="step-<?php echo $cantidad_2; ?>" style="z-index:1000; line-height: 160%;" style="background-color: #b5b4b4">
										<form name="forma<?php echo $cantidad_2; ?>" class="form-horizontal form-label-left">
												<?php
													echo "<div align='center' style='background: #fff;position: relative; border: solid 2px; border-color: red; height:200px; width: 850px; border-radius: 15px 50px 30px; margin: 5px; padding: 5px;'>";
													...
													foreach ($data_opciones as $data_opciones) {
														...
														echo "
																<tr onmouseover='this.bgColor=\"#c8c8c8\"' onmouseout='this.bgColor=\"#ffffff\"'>
																	<td align='right'>$opcion&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
																	<td align='left'>  (+) <input class='radio_iol' $selected1 type='radio' name='mas_".$data->id_preguntas."' id='mas_".$data->id_preguntas."' value='".$data_opciones->id_opciones."' onclick='colocar_respuesta(\"iep_mas\", \"\",\"\",".$data->id_preguntas.", ".$data_opciones->id_opciones.",\"".$opcion."\",\"\")'></td>
																	<td align='left'>  (-) <input class='radio_iol' $selected2 type='radio' name='menos_".$data->id_preguntas."' id='menos_".$data->id_preguntas."' value='".$data_opciones->id_opciones."' onclick='colocar_respuesta(\"iep_menos\", \"\",\"\",".$data->id_preguntas.", ".$data_opciones->id_opciones.",\"".$opcion."\",\"\")'></td>
																</tr>";
														$k++;
													}
													echo "</table></td></tr>";
